...
import asyncio, json, os, platform, socket, sys, tempfile, time, urllib.request
...
if IS_WINDOWS:
    SOCK_HOST = "127.0.0.1"
    SOCK_PORT = 9223 + hash(NAME) % 10000
    SOCK = (SOCK_HOST, SOCK_PORT)
    SOCK_PATH = os.path.join(TEMP_DIR, f"bu-{NAME}.port")
else:
    SOCK = f"/tmp/bu-{NAME}.sock"
    SOCK_PATH = SOCK

LOG = os.path.join(TEMP_DIR, f"bu-{NAME}.log")
PID = os.path.join(TEMP_DIR, f"bu-{NAME}.pid")
BUF = 500
PROFILES = [
    ...
    async def start(self):
        ...
        async def tap(method, params, session_id=None):
            ...
            if method == "Page.javascriptDialogOpening":
                ...
                try:
                    await asyncio.wait_for(self.cdp.send_raw("Runtime.evaluate", {"expression": mark_js}, session_id=self.session), timeout=2)
                except Exception:
                    pass
...
async def serve(d):
    ...
    if IS_WINDOWS:
        ...
        log(f"listening on {SOCK_HOST}:{SOCK_PORT} (name={NAME}, remote={REMOTE_ID or 'local'})")
    else:
...
if __name__ == "__main__":
    if already_running():
        if IS_WINDOWS:
            print(f"daemon already running on {SOCK_HOST}:{SOCK_PORT}", file=sys.stderr)
        else:
